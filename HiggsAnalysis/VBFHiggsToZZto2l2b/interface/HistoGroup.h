#ifndef HiggsAnalysis_VBFHiggsToZZto2l2b_HistoGroup_h
#define HiggsAnalysis_VBFHiggsToZZto2l2b_HistoGroup_h

//------------------------------------------------------------
// Title: HistoGroup.h
// Purpose: To histogram data objects deriving from Object
...
#include "HiggsAnalysis/VBFHiggsToZZto2l2b/interface/PhysVarHisto.h"
...
namespace vbfhzz2l2b {
  ...
  protected:
    ...
    PhysVarHisto * h_invMassLeading_;   //!< invariant mass of 2 leading objs >
    ...
    h_pt_(0), h_eta_(0), h_phi_(0), h_mass_(0)
  {
    ...
    name  = prepend_+"invMassLeading"; title = "invariant mass of leading "+groupName_+"s";
    addHisto( h_invMassLeading_ = new PhysVarHisto( name.c_str(), title.c_str(),  nBins_, 0., 1000., currDir_, "", "vD" ) );
  ...
  fillCollection( const std::vector<PHYS_OBJECT> & coll, double weight )
  {
    ...
    double invMass = 0.;
    if (coll.size() < 2) invMass = coll[0].p4().mass();
    else invMass = (coll[0].p4()+coll[1].p4()).mass();
    h_invMassLeading_->fill( invMass, 1, weight);
