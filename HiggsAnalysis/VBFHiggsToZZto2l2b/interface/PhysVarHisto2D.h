#ifndef HiggsAnalysis_VBFHiggsToZZto2l2b_PhysVarHisto2D_h
#define HiggsAnalysis_VBFHiggsToZZto2l2b_PhysVarHisto2D_h 1
...
namespace vbfhzz2l2b {
  ...
  class PhysVarHisto2D
  {
  public:
    PhysVarHisto2D( std::string name,
	       std::string title,
	       int         xnbins,
	       double      xlow,
	       double      xhigh,
    ...
    virtual ~PhysVarHisto2D() { };  //!  Note we don't delete histograms!

    //--- Make one TH1 (may need more than one); all decorations should be done in this call.
    virtual void makeTH2(int nameId = -1);
    ...
    template <class T>
      void xvec(std::vector<T> & xretVec) {
      xretVec.resize( xvalueColl_.size() );
      for ( unsigned int i = 0; i < xvalueColl_.size(); i++ )
	xretVec[i] = static_cast<T>( xvalueColl_[i] );
    } //!< vector of current values in a list
    template <class T>
      void yvec(std::vector<T> & yretVec) {
      yretVec.resize( yvalueColl_.size() );
      for ( unsigned int i = 0; i < yvalueColl_.size(); i++ )
	yretVec[i] = static_cast<T>( yvalueColl_[i] );
    } //!< vector of current values in a list
  ...
  private:
    ...
    std::vector<TH2D *> histos_ ;   // maybe use a base class TH1* ?
