...
class THStackLegend {
 ...
 public:
  ...
  void Print(TString OPTION = "",TString FILE = "");
...
template<class T>
void THStackLegend<T>::SetLogY() const {
  canvas_->GetPad(0)->SetLogy();
}
...
template<class T>
void THStackLegend<T>::Print(TString OPTION,TString FILE) {
  if ( !drawn ) {
    canvas_->cd();
    stack_->Draw(OPTION);
    legend_->Draw();
