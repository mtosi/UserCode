...
//  algos.push_back("softElectronBJetTags");
  ...
  for(int i = 0 ; i < algos.size() ; i++)
   {
      cout << algos[i] << endl;
      //TH1F * h = (TH1F *) getHistogram(files[0],algos[i],"FlavEffVsBEff_DUS_discr","ETA_0-1.4");
    TH1F * h = (TH1F *) getHistogram(files[0],algos[i],"FlavEffVsBEff_DUS_discr","GLOBAL");
    //     TH1F * h = (TH1F *) getHistogram(files[0],algos[i],"FlavEffVsBEff_C_discr","GLOBAL");
     //TH1F * h = (TH1F *) getHistogram(files[0],algos[i],"FlavEffVsBEff_DUS_discr","ETA_1.4-2.4");
     cout << h << endl;
