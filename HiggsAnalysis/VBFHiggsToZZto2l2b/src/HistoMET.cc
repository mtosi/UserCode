...
HistoMET::HistoMET( std::string dir, std::string group,std::string pre,
  ...
  : HistoGroup<CaloMET>( dir, group, pre, pt1, pt2, m1, m2, parentDir)
{
...
void HistoMET::fill( const CaloMET * met, uint iPart, double weight)
{

  // First fill common 4-vector histograms
  HistoGroup<CaloMET>::fill( met, iPart, weight );
...
void HistoMET::fill( const reco::ShallowClonePtrCandidate * pshallow, uint iPart, double weight)
{
  ...
  const CaloMET * met = dynamic_cast<const CaloMET*>(&*(pshallow->masterClonePtr()));
  ...
  HistoGroup<CaloMET>::fill( pshallow, iPart, weight);
...
void HistoMET::fillCollection( const std::vector<CaloMET> & coll, double weight ) 
{
 
  h_size_->fill( coll.size(), 1, weight );     //! Save the size of the collection.

  std::vector<CaloMET>::const_iterator
    iobj = coll.begin(),
    iend = coll.end();
