...
#include "HiggsAnalysis/VBFHiggsToZZto2l2b/interface/PhysVarHisto2D.h"

#include <string>
#include <sstream>
#include <iostream>

using vbfhzz2l2b::PhysVarHisto2D;

// Constructor:
PhysVarHisto2D::PhysVarHisto2D( std::string name,
				std::string title,
				int         xnbins,
				double      xlow,
				double      xhigh,
...
{
  if (verboseLevel_ > 5)
    std::cout << "PhysVarHisto2D(" << name_ << "):: in constructor." << std::endl;

  // &&& Not sure what (if anything) will be needed to deal with the ntuple.
}


void PhysVarHisto2D::makeTH2(int nameId) {
  if (verboseLevel_ > 5)
    std::cout << "PhysVarHisto2D(" << name_ << "):: in makeTH2()." << std::endl;


  if (saveHist_ && currDir_) {
    if (verboseLevel_ > 5)
      std::cout << "PhysVarHisto2D(" << name_ << ")::makeTH2:"
    ...
    TH2D * h = currDir_->make<TH2D>( name.c_str(), title.c_str(),
				     xnbins_, xlow_, xhigh_,
				     ynbins_, ylow_, yhigh_ );
...
// call to fill().
void
PhysVarHisto2D::fill( double x, double y, unsigned int imulti, double weight ) {
  // Once this PhysVarHisto2D was made, then there's no turning back
  ...
  if (verboseLevel_ > 5) {  // very verbose
    std::cout << "PhysVarHisto2D(" << name_ << ")::fill:"
	      << " xvalue = " << xvalue_ 
	      << " yvalue = " << yvalue_ 
  ...
  if ( ! saveHist_ ) {
    std::cout << "PhysVarHisto2D(" << name_ << ")::fill: saveHist is not defined" << std::endl;
    return;
  }
  ...
  if ( !isMC_ ) {
    while ( imulti > histos_.size() ) {
      if (verboseLevel_ > 4) {
	std::cout << "PhysVarHisto2D(" << name_ << ")::fill: grow histo list by one."
		  << std::endl;
      }
      //--- Make another TH2 at index == current value of size()
      makeTH2();    // histos_ vector grows by one element
    }
    if ( verboseLevel_ > 4 )
      std::cout << "PhysVarHisto2D(" << name_ << ")::fill: About to fill " << imulti << std::endl;
    histos_[ imulti-1 ]->Fill( xvalue_, yvalue_, weight );
    if ( verboseLevel_ > 4 )
      std::cout << "PhysVarHisto2D(" << name_ << ")::fill: Done filling " << imulti << std::endl;
  } 
  else {
