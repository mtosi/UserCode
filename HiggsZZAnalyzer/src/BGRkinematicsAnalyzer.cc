...
#include "AnalysisExamples/HiggsZZAnalyzer/interface/BGRkinematicsAnalyzer.h"
...
BGRkinematicsAnalyzer::BGRkinematicsAnalyzer(const edm::ParameterSet& iConfig) :
  conf_( iConfig ),
...
BGRkinematicsAnalyzer::~BGRkinematicsAnalyzer()
{
...
void
BGRkinematicsAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  using namespace edm;
  ...
  if ( ( globalMuonsSize >= nZleptons_ || simpleElectronsSize >= nZleptons_ ) &&
    ...
    if ( ZlepVec.size() >= nZleptons_ && ZlepVec[0].pt() != 0. && ZlepVec[1].pt() != 0. ) {
      ...
      //      if ( partonsCandidates.size() >= nZjets_ ) {
...
//		std::cout << "ZlepVec[1].pt: " << ZlepVec[1].pt() << " eta: " << ZlepVec[1].eta() << " phi: " << ZlepVec[1].phi() << std::endl;
		...
		for ( ; jet1 != aboveEtCutJetVec.end()-1; ++jet1, i1++ ) {
		  ...
		  for ( ; jet2 != aboveEtCutJetVec.end(); ++jet2, i2++) {
			...
			//math::XYZTLorentzVector jet4Vec = 
			//  math::XYZTLorentzVector((jet4->p4()).px(),(jet4->p4()).py(),(jet4->p4()).pz(),(jet4->p4()).e());
	...
	//      } // end if partonsCandidates.size() >= nZjets_
...
void 
BGRkinematicsAnalyzer::beginJob(const edm::EventSetup&)
{
...
DEFINE_FWK_MODULE(BGRkinematicsAnalyzer);
