  ...
  HMassCut_(                   iConfig.getUntrackedParameter<double>(        "HMassCut"                     ) )
{
  ...
  ZbeforeDeltaRcutCounter_ = 0;
  ZafterDeltaRcutCounter_  = 0;
  ZbeforeLeptonShiftedEtaCutCounter_ = 0;
  ZafterLeptonShiftedEtaCutCounter_  = 0;
...
//	  double tmpOfflinejetTKtagmassS2  = offlinejet->tagTkMassS2();
	  ...
	  if ( goodMuonVec.size() >= nZleptons_ ) {
	    ...
	    for ( ; goodMuonVec_itr != goodMuonVec.end(); ++goodMuonVec_itr ) {
	      ...
	      if ( tmp_jetlepton_deltaR < centralJetLeptonDeltaRCut_ ) continue;
	    }
...
//	  if (tmpOfflinejetTKtagmassS2  != 0. ) OfflinejetTKtagmassS2_->Fill(  tmpOfflinejetTKtagmassS2  );
	...
	double forwardDiJetDeltaEta = VBFsystemVec[0].eta()-VBFsystemVec[1].eta();
	double forwardDiJetDeltaPhi = DeltaPhi(VBFsystemVec[0].phi(),VBFsystemVec[1].phi());
    ...
    if ( goodIc5centralJetVec.size() >= nZjets_ ) {
      std::vector<OfflineJet> maxEtJets;
      maxEtJets.push_back(null_offlinejet_);
      maxEtJets.push_back(null_offlinejet_);
      // hadronic Z
      // ----------
	...
	for ( ; goodIc5centralJetVec_itr2 != goodIc5centralJetVec.end(); ++goodIc5centralJetVec_itr2 ) {
	  ...
	  if (goodIc5centralJetVec_itr1->et() >= maxEtJets[0].et())      maxEtJets[0]=*goodIc5centralJetVec_itr1;
	  else if (goodIc5centralJetVec_itr1->et() >= maxEtJets[1].et()) maxEtJets[1]=*goodIc5centralJetVec_itr1;
	  if (goodIc5centralJetVec_itr2->et() >= maxEtJets[0].et())      maxEtJets[0]=*goodIc5centralJetVec_itr2;
	  else if (goodIc5centralJetVec_itr2->et() >= maxEtJets[1].et()) maxEtJets[1]=*goodIc5centralJetVec_itr2;
	}
      }
    
      if (maxEtJets.size() == nZjets_) {
	double scaleJet1 = maxEtJets[0].et()/maxEtJets[0].uncorrEt();
	double scaleJet2 = maxEtJets[1].et()/maxEtJets[1].uncorrEt();
	double invMass = (maxEtJets[0].p4()*scaleJet1+maxEtJets[1].p4()*scaleJet2).M();
	double deltaR = TMath::Sqrt(pow(maxEtJets[0].eta()-maxEtJets[1].eta(),2)
				    +pow(DeltaPhi(&maxEtJets[0],&maxEtJets[1]),2));
	if ( VBFsystemVec.size() == nforwardjets_ && systemVBFrec ) {
	  double jet1ShiftedEta = fabs(maxEtJets[0].eta()-1/(2*(VBFsystemVec[0].eta()+VBFsystemVec[1].eta())));	
	  double jet2ShiftedEta = fabs(maxEtJets[1].eta()-1/(2*(VBFsystemVec[0].eta()+VBFsystemVec[1].eta())));	
	  recJetsInvMassVSjetShiftedEta_->Fill(invMass,jet1ShiftedEta);
	  recJetsInvMassVSjetShiftedEta_->Fill(invMass,jet2ShiftedEta);
	}
	recJetsInvMass_->Fill(invMass);
	recJetsInvMassVSdeltaR_->Fill(invMass,deltaR);
      }
	...
	if ( lepton1ShiftedEta <= ZdileptonShiftedEtaCut_ && lepton2ShiftedEta <= ZdileptonShiftedEtaCut_ ) {
      ...
      // leptonic Z cuts
      if ( DiLeptonMass>=ZdileptonMassminCut_ && DiLeptonMass<=ZdileptonMassmaxCut_ ) {
	recZllMass_leptonicZcutsAFTERZdileptonMass->Fill(DiLeptonMass);	     
	recHlljj_leptonicZcutsAFTERZdileptonMass->Fill(lljjInvMass);	     
	if ( leptonDeltaR>=ZdileptonDeltaRminCut_ && leptonDeltaR<=ZdileptonDeltaRmaxCut_ ) {
	  recZllMass_leptonicZcutsAFTERZleptonDeltaR->Fill(DiLeptonMass);	     
	  recHlljj_leptonicZcutsAFTERZleptonDeltaR->Fill(lljjInvMass);	     
	  // hadronic Z
	  if ( centralDiJetMass>=ZdijetMassminCut_ && centralDiJetMass<=ZdijetMassmaxCut_ ) {
	    recZjjMass_hadronicZcutsAFTERZdijetMass->Fill(centralDiJetMass);	     
	    recHlljj_hadronicZcutsAFTERZdijetMass->Fill(lljjInvMass);	     
	    if ( centralJetDeltaR>=ZdijetDeltaRminCut_ && centralJetDeltaR<=ZdijetDeltaRmaxCut_ ) {
	      recZjjMass_hadronicZcutsAFTERZjetDeltaR->Fill(centralDiJetMass);	     
	      recHlljj_hadronicZcutsAFTERZjetDeltaR->Fill(lljjInvMass);	    
	      recFWDjj_hadronicZcutsAFTERjetDeltaR->Fill(VBFdiJetMass);
	      recZleptonsShitedEta_hadronicZcutsAFTERjetDeltaR->Fill(lepton1ShiftedEta);
	      recZleptonsShitedEta_hadronicZcutsAFTERjetDeltaR->Fill(lepton2ShiftedEta);
	      recFWDjjPtBalance_hadronicZcutsAFTERjetDeltaR->Fill(pTbalance);
	      // FWD jets system
	      if ( lepton1ShiftedEta<=ZdileptonShiftedEtaCut_  && lepton2ShiftedEta<=ZdileptonShiftedEtaCut_ ) {
		recHlljj_FWDcutsAFTERleptonsShiftedEta->Fill(lljjInvMass);	    
		if ( centralJet1ShiftedEta<=ZdijetShiftedEtaCut_ && centralJet2ShiftedEta<=ZdijetShiftedEtaCut_ ) {
		  recHlljj_FWDcutsAFTERjetsShiftedEta->Fill(lljjInvMass);
		  if ( VBFdiJetMass >= forwardDiJetMassCut_ ) {
		    recHlljj_FWDcutsAFTERforwardDiJetMass->Fill(lljjInvMass);	     
		    if ( pTbalance <= pTbalanceCut_ ) {
		      recHlljj_FWDcutsAFTERpTbalance->Fill(lljjInvMass);	     
		      recHZZdeltaR_FWDcutsAFTERpTbalance->Fill(recHZZdeltaR);
		      if ( recHZZdeltaR>=ZZdeltaRminCut_ && recHZZdeltaR<=ZZdeltaRmaxCut_ ) recHlljj_final_->Fill(lljjInvMass);
		    }
		  }
		}
	      }
...
HiggsZZAnalyzer::beginJob(const edm::EventSetup&)
{
  ...
  ZbeforeDeltaRcutNumber_ = new TH1D("ZbeforeDeltaRcutNumber","number of leptonic Z before the #DeltaR cut",1,0.,1.);
  ZafterDeltaRcutNumber_  = new TH1D("ZafterDeltaRcutNumber","number of leptonic Z after the #DeltaR cut",1,0.,1.);
  ZbeforeLeptonShiftedEtaCutNumber_ = new TH1D("ZbeforeLeptonShiftedEtaCutNumber","number of leptonic Z before the shifted #eta cut",1,0.,1.);
  ZafterLeptonShiftedEtaCutNumber_ = new TH1D("ZafterLeptonShiftedEtaCutNumber","number of leptonic Z after the shifted #eta cut",1,0.,1.);
...
//  OfflinejetTKtagmassS2_        = new TH1D("OfflinejetTKtagmassS2",       "offline jet tagmass from tracks S2",               nbin_,   0.,        10.);
  ...
  OfflinejetLeptonDeltaR_ = new TH1D("OfflinejetLeptonDeltaR","#DeltaR between the jet and lepton",        nbin_,0.,5.);
  centralJetLeptonDeltaR_ = new TH1D("centralJetLeptonDeltaR","#DeltaR between the central jet and lepton",nbin_,0.,5.);

  forwardJetEt_           = new TH1D("forwardjetEt",          "offline forward jets E_{T}",           nbin_,  0., 5000.);
  forwardJetLeptonDeltaR_ = new TH1D("forwardJetLeptonDeltaR","#DeltaR beteen forward jet and lepton",nbin_,  0.,    5.);
  ...
  forwardDiJetMass_       = new TH1D("forwardDiJetMass",      "di-jet mass for forward jets",         nbin_,  0., 6000.);
  ...
  forwardDiJetDeltaR_     = new TH1D("forwardDiJetDeltaR",    "#DeltaR between the 2 forward jets",   nbin_,  0.,    5.);
  ...
  dileptonMassVSdeltaR_ = new TH2D("dileptonmassVSdeltaR","di-lepton mass VS #DeltaR",nbin_,0.,200.,nbin_,0.,5.);
  dijetMassVSdeltaR_    = new TH2D("dijetmassVSdeltaR",   "di-jet mass VS #DeltaR",   nbin_,0.,200.,nbin_,0.,5.);
  dibjetMassVSdeltaR_   = new TH2D("dibjetmassVSdeltaR",  "di-bjet mass VS #DeltaR",  nbin_,0.,200.,nbin_,0.,5.);
  ...
  recZbbMass_           = new TH1D("recZbbMass",          "reconstructed dib-jet invariant mass",              nbin_,  0.,200.);
  recZllMassResolution_ = new TH1D("recZllMassResolution","reconstructed dilepton invariant mass resolution",  nbin_, -1.,  1.);
  recZeeMassResolution_ = new TH1D("recZeeMassResolution","reconstructed dielectron invariant mass resolution",nbin_, -1.,  1.);
  recZmmMassResolution_ = new TH1D("recZmmMassResolution","reconstructed dimuon invariant mass resolution",    nbin_, -1.,  1.);
  recZjjMassResolution_ = new TH1D("recZjjMassResolution","reconstructed dijet invariant mass resolution",     nbin_, -1.,  1.);
  recZbbMassResolution_ = new TH1D("recZbbMassResolution","reconstructed dib-jet invariant mass resolution",   nbin_, -1.,  1.);
  ...
  recZllDeltaR_         = new TH1D("recZllDeltaR",        "#DeltaR between the reconstruced leptons from Z",    nbin_,  0.,  5.);
  recZjjDeltaR_         = new TH1D("recZjjDeltaR",        "#DeltaR between the reconstruced jets from Z",       nbin_,  0.,  5.);
  recZbbDeltaR_         = new TH1D("recZbbDeltaR",        "#DeltaR between the reconstruced b-jets from Z",     nbin_,  0.,  5.);
  ...
  recZjjLeptonDeltaR_ = new TH1D("recZjjLeptonDeltaR","#DeltaR between the jet from Z and the lepton",  nbin_,0.,5.);
  recZbbLeptonDeltaR_ = new TH1D("recZbbLeptonDeltaR","#DeltaR between the b-jet from Z and the lepton",nbin_,0.,5.);

  recJetsInvMass_ = new TH1D("recJetsInvMass","invariant mass of the 2 leading jets in the central region",nbin_,0.,600.);
  recJetsInvMassVSjetShiftedEta_ = new TH2D("recJetsInvMassVSjetShiftedEta","invariant mass of the 2 leading jets VS shifted #eta",nbin_,0.,200.,nbin_,0.,10.);
  recJetsInvMassVSdeltaR_        = new TH2D("recJetsInvMassVSdeltaR","invariant mass of the 2 leading jets VS #DeltaR",nbin_,0.,200.,nbin_,0.,5.);
  ...
  recHlljjMassResolution_ = new TH1D("recHlljjMassResolution","reconstructed dilepton+dijet invariant mass resolution",  nbin_,-1.,1.);
  recHllbbMassResolution_ = new TH1D("recHllbbMassResolution","reconstructed dilepton+dib-jet invariant mass resolution",nbin_,-1.,1.);

  
  recZllMass_leptonicZcutsAFTERZdileptonMass       = new TH1D("recZllMass_leptonicZcutsAFTERZdileptonMass",      "reconstructed m_{ll} after leptonic Z mass cut",                nbin_,0., 200.);
  recHlljj_leptonicZcutsAFTERZdileptonMass         = new TH1D("recHlljj_leptonicZcutsAFTERZdileptonMass",        "reconstructed m{lljj} after leptonic Z mass cut",               nbin_,0.,1000.);
  recZllMass_leptonicZcutsAFTERZleptonDeltaR       = new TH1D("recZllMass_leptonicZcutsAFTERZleptonDeltaR",      "reconstructed m_{ll} after #DeltaR_{ll} cut",                   nbin_,0., 200.);
  recHlljj_leptonicZcutsAFTERZleptonDeltaR         = new TH1D("recHlljj_leptonicZcutsAFTERZleptonDeltaR",        "reconstructed m_{lljj} after #DeltaR_{ll} cut",                 nbin_,0.,1000.);
  recZjjMass_hadronicZcutsAFTERZdijetMass          = new TH1D("recZjjMass_hadronicZcutsAFTERZdijetMass",         "reconstructed m_{jj} after hadronic Z mass cut",                nbin_,0., 200.);
  recHlljj_hadronicZcutsAFTERZdijetMass            = new TH1D("recHlljj_hadronicZcutsAFTERZdijetMass",           "reconstructed m_{lljj} after hadronic Z mass cut",              nbin_,0.,1000.);
  recZjjMass_hadronicZcutsAFTERZjetDeltaR          = new TH1D("recZjjMass_hadronicZcutsAFTERZjetDeltaR",         "reconstructed m_{jj} after #DeltaR_{jj} cut",                   nbin_,0., 200.);
  recHlljj_hadronicZcutsAFTERZjetDeltaR            = new TH1D("recHlljj_hadronicZcutsAFTERZjetDeltaR",           "reconstructed m_{lljj} after #DeltaR_{jj} cut",                 nbin_,0.,1000.);
  recFWDjj_hadronicZcutsAFTERjetDeltaR             = new TH1D("recFWDjj_hadronicZcutsAFTERjetDeltaR",            "forward m_{jj} after #DeltaR_{jj} cut",                         nbin_,0.,6000.);
  recZleptonsShitedEta_hadronicZcutsAFTERjetDeltaR = new TH1D("recZleptonsShitedEta_hadronicZcutsAFTERjetDeltaR","leptons shifted #eta after #DeltaR_{jj} cut",                   nbin_,0.,  10.);
  recFWDjjPtBalance_hadronicZcutsAFTERjetDeltaR    = new TH1D("recFWDjjPtBalance_hadronicZcutsAFTERjetDeltaR",    "balance p_{T} after #DeltaR_{jj} cut",                         nbin_,0.,1000.);
  recHlljj_FWDcutsAFTERleptonsShiftedEta           = new TH1D("recHlljj_FWDcutsAFTERleptonsShiftedEta",           "reconstructed m_{lljj} after leptons shifted #eta cut",        nbin_,0.,1000.);
  recHlljj_FWDcutsAFTERjetsShiftedEta              = new TH1D("recHlljj_FWDcutsAFTERjetsShiftedEta",              "reconstructed m_{lljj} after jets shifted #eta cut",           nbin_,0.,1000.);
  recHlljj_FWDcutsAFTERforwardDiJetMass            = new TH1D("recHlljj_FWDcutsAFTERforwardDiJetMass",            "reconstructed m_{lljj} after forward m_{jj} cut",              nbin_,0.,1000.);
  recHlljj_FWDcutsAFTERpTbalance                   = new TH1D("recHlljj_FWDcutsAFTERpTbalance",                   "reconstructed m_{lljj} after balance p_{T} cut",               nbin_,0.,1000.);
  recHZZdeltaR_FWDcutsAFTERpTbalance               = new TH1D("recHZZdeltaR_FWDcutsAFTERpTbalance",               "#DeltaR between the 2 reconstructed Z after balance p_{T} cut",nbin_,0.,  10.);
  recHlljj_final_                                  = new TH1D("recHlljj_final",                                   "reconstructed m_{lljj} after all cuts",                        nbin_,0.,1000.);
  ...
  DeltaRZZ_   = new TH1D("DeltaRZZ",   "#DeltaR between the 2 Z",   nbin_,  0.,5.);   
...
HiggsZZAnalyzer::endJob() {
  ...
  std::cout << "ZbeforeDeltaRcutCounter_ : " << ZbeforeDeltaRcutCounter_ << std::endl;
  std::cout << "ZafterDeltaRcutCounter_: " << ZafterDeltaRcutCounter_ << std::endl;
  double effDeltaRcut = double(ZafterDeltaRcutCounter_)/double(ZbeforeDeltaRcutCounter_);
  std::cout << "effDeltaRcut: " << effDeltaRcut << " +- "
	    << TMath::Sqrt(effDeltaRcut*(1-effDeltaRcut)/ZbeforeDeltaRcutCounter_) << std::endl;
  std::cout << "ZbeforeLeptonShiftedEtaCutCounter_ : " << ZbeforeLeptonShiftedEtaCutCounter_ << std::endl;
  std::cout << "ZafterLeptonShiftedEtaCutCounter_: " << ZafterLeptonShiftedEtaCutCounter_ << std::endl;
  double effLeptonShitedEtacut = double(ZafterLeptonShiftedEtaCutCounter_)/double(ZbeforeLeptonShiftedEtaCutCounter_);
  std::cout << "effLeptonShitedEtacut: " << effLeptonShitedEtacut << std::endl;
  ...
  ZbeforeDeltaRcutNumber_->SetBinContent(1,ZbeforeDeltaRcutCounter_);
  ZafterDeltaRcutNumber_->SetBinContent(1,ZafterDeltaRcutCounter_);
  ZbeforeLeptonShiftedEtaCutNumber_->SetBinContent(1,ZbeforeLeptonShiftedEtaCutCounter_);
  ZafterLeptonShiftedEtaCutNumber_->SetBinContent(1,ZafterLeptonShiftedEtaCutCounter_);
  ZbeforeDeltaRcutNumber_->Write();
  ZafterDeltaRcutNumber_->Write();
  ZbeforeLeptonShiftedEtaCutNumber_->Write();
  ZafterLeptonShiftedEtaCutNumber_->Write();
...
//  OfflinejetTKtagmassS2_->Write();
  ...
  recJetsInvMass_->Write();
  recJetsInvMassVSjetShiftedEta_->Write();
  recJetsInvMassVSdeltaR_->Write();
  ...
  recZllMass_leptonicZcutsAFTERZdileptonMass->Write(); 
  recHlljj_leptonicZcutsAFTERZdileptonMass->Write();
  recZllMass_leptonicZcutsAFTERZleptonDeltaR->Write(); 
  recHlljj_leptonicZcutsAFTERZleptonDeltaR->Write(); 
  recZjjMass_hadronicZcutsAFTERZdijetMass->Write(); 
  recHlljj_hadronicZcutsAFTERZdijetMass->Write(); 
  recZjjMass_hadronicZcutsAFTERZjetDeltaR->Write(); 
  recHlljj_hadronicZcutsAFTERZjetDeltaR->Write(); 
  recFWDjj_hadronicZcutsAFTERjetDeltaR->Write(); 
  recZleptonsShitedEta_hadronicZcutsAFTERjetDeltaR->Write(); 
  recFWDjjPtBalance_hadronicZcutsAFTERjetDeltaR->Write(); 
  recHlljj_FWDcutsAFTERleptonsShiftedEta->Write(); 
  recHlljj_FWDcutsAFTERjetsShiftedEta->Write(); 
  recHlljj_FWDcutsAFTERforwardDiJetMass->Write(); 
  recHlljj_FWDcutsAFTERpTbalance->Write(); 
  recHZZdeltaR_FWDcutsAFTERpTbalance->Write(); 
  recHlljj_final_->Write(); 
